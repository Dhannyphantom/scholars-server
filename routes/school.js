...
/**
 * POST /api/school/:schoolId/classes/:classLevel/transfer
 * Transfer a single student or all students to another class level
 * Body: { studentId?: string, targetLevel: string, upgradeAll?: boolean }
 */
router.post(
  "/:schoolId/classes/:classLevel/transfer",
  auth,
  async (req, res) => {
    try {
      const { schoolId, classLevel } = req.params;
      const userId = req.user.userId;
      const { studentId, targetLevel, upgradeAll } = req.body;
      ...
      if (!targetLevel) {
        return res
          .status(400)
          .json({ success: false, message: "Target level is required" });
      }

      if (!CLASS_ORDER.includes(classLevel.toLowerCase())) {
        return res
          .status(400)
          .json({ success: false, message: "Invalid source class level" });
      }

      if (!CLASS_ORDER.includes(targetLevel.toLowerCase())) {
        return res
          .status(400)
          .json({ success: false, message: "Invalid target class level" });
      }
      ...
      if (!sourceClass) {
        return res
          .status(404)
          .json({ success: false, message: "Source class not found" });
      }

      const targetClass = school.classes.find(
        (c) => c.level === targetLevel.toLowerCase(),
      );
      ...
      if (!targetClass) {
        return res.status(404).json({
          success: false,
          message:
            "Target class not found. Please create the target class first.",
        });
      }
      ...
      if (upgradeAll) {
        studentsToTransfer = [...sourceClass.students];
        ...
        studentsToTransfer = [studentId];
        transferCount = 1;
      }

      // Update class level and class.id on User documents
      await User.updateMany(
        { _id: { $in: studentsToTransfer } },
        {
          $set: {
            "class.level": targetClass.level,
            "class.id": targetClass._id,
      ...
      res.json({
        success: true,
        message: `Successfully transferred ${transferCount} student${transferCount > 1 ? "s" : ""} from ${sourceClass.level.toUpperCase()} to ${targetClass.level.toUpperCase()}${targetClass.alias ? ` (${targetClass.alias})` : ""}`,
        data: {
          studentsTransferred: transferCount,
          sourceClass: {
            id: sourceClass._id,
